...
# the damian user, and ensure file permissions, grant sudo rights, and such
...
if [ "$(id -u)" == 0 ] ; then
    ...
    # Refit the damian user to the desired the user (NB_USER)
    if id damian &> /dev/null ; then
        if ! usermod --home "/home/${NB_USER}" --login "${NB_USER}" damian 2>&1 | grep "no changes" > /dev/null; then
            _log "Updated the damian user:"
            _log "- username: damian       -> ${NB_USER}"
            _log "- home dir: /home/damian -> /home/${NB_USER}"
        fi
    elif ! id -u "${NB_USER}" &> /dev/null; then
        _log "ERROR: Neither the damian user or '${NB_USER}' exists. This could be the result of stopping and starting, the container with a different NB_USER environment variable."
        exit 1
    fi
    ...
    # Move or symlink the damian home directory to the desired users home
    # directory if it doesn't already exist, and update the current working
    # directory to the new location if needed.
    if [[ "${NB_USER}" != "damian" ]]; then
        if [[ ! -e "/home/${NB_USER}" ]]; then
            _log "Attempting to copy /home/damian to /home/${NB_USER}..."
            mkdir "/home/${NB_USER}"
            if cp -a /home/damian/. "/home/${NB_USER}/"; then
                _log "Success!"
            else
                _log "Failed to copy data from /home/damian to /home/${NB_USER}!"
                _log "Attempting to symlink /home/damian to /home/${NB_USER}..."
                if ln -s /home/damian "/home/${NB_USER}"; then
                    _log "Success creating symlink!"
                else
                    _log "ERROR: Failed copy data from /home/damian to /home/${NB_USER} or to create symlink!"
                    exit 1
                fi
            fi
        fi
        # Ensure the current working directory is updated to the new path
        if [[ "${PWD}/" == "/home/damian/"* ]]; then
    ...
    run-hooks /usr/local/bin/before-start-root.d
...
# user we started as.
    ...
    DAMIAN_UID="$(id -u damian 2>/dev/null)"  # The default UID for the damian user
    DAMIAN_GID="$(id -g damian 2>/dev/null)"  # The default GID for the damian user
    ...
    if ! whoami &> /dev/null; then
        ...
        if [[ -w /etc/passwd ]]; then
            _log "Renaming old damian user to nayvoj ($(id -u damian):$(id -g damian))"
            ...
            sed --expression="s/^damian:/nayvoj:/" /etc/passwd > /tmp/passwd
            echo "${NB_USER}:x:$(id -u):$(id -g):,,,:/home/damian:/bin/bash" >> /tmp/passwd
            ...
            if [[ "${NB_USER}" != "damian" ]]; then
                _log "WARNING: user is ${NB_USER} but home is /home/damian. You must run as root to rename the home directory!"
            fi
        else
    ...
    if [[ ! -w /home/damian ]]; then
        _log "WARNING: no write access to /home/damian. Try starting the container with group 'users' (100), e.g. using \"--group-add=users\"."
    fi

    # NOTE: This hook is run as the user we started the container as!
    run-hooks /usr/local/bin/before-start-user.d
    unset_explicit_env_vars
